...
- **Dot touch targets too small** — 5px × 5px dots. Minimum should be 44×44pt (Apple) or 48×48dp (Material). Basically untappable.
...
### Critical
- **No offline/cached data** — Close app = lose everything. No AsyncStorage, no SQLite. Reopening forces full re-fetch + wait.
...
### Critical
- **Zero memoization** — No `React.memo` on any screen component. Every parent re-render cascades to all children even with identical props.
...
### Important
- ForecastScreen recalculates min/max/range on every render (should `useMemo`)
...
### Critical
- **No error boundaries** — A single thrown error crashes the entire app. Zero recovery.
...
- **Weak error handling** — Generic catch-all. Doesn't distinguish 401 (bad key) vs 429 (rate limit) vs 503 (down).
...
- Widget spawns raw Thread — should use WorkManager. Thread can hang indefinitely.
- Zero test coverage
...
### Critical
- **No persistence layer** — No AsyncStorage, no SQLite, no file cache. Everything lost on close.
...
- Widget geocoding called every update even if lat/lon unchanged — should cache
...
### Important
- **No layer transition animations** — Scroll just snaps. No parallax, crossfade, or animated backgrounds.
- **Generic loading state** — Spinner + static text. No skeleton loaders or content preview.
- **Error messages show raw API text** — Should be plain-English with suggestions.
- **No state persistence** — Reopen app = always starts at NowScreen. Should remember last layer.
- **No micro-interactions** — Button presses, layer changes, data refreshes give zero feedback.

### Nice-to-have
- Rotating loading tips ("Checking clouds...", "Measuring humidity...")
- Seasonal color theming
- Animated weather effects (rain, clouds, sun rays)
- Expressive descriptions ("Crisp autumn morning" instead of "Partly Cloudy")
...
### Critical
- **No periodic refresh** — `updatePeriodMillis` in XML metadata is set, but widget only really updates from app sync. If app not opened for days, widget data rots.
- **Raw Thread instead of WorkManager** — Thread inside `onUpdate()` can hang widget system. Should use WorkManager for background API calls.
- **No independent refresh capability** — If user clears app data, widget stuck forever with stale/no data.
- **No configuration activity** — Can't customize what the widget shows. Single fixed layout.

### Important
- Layout not responsive to resize — content clips if widget resized
- No weather icons — text only, looks plain compared to other widget apps
- Long location names overflow ("San Francisco-Oakland Metropolitan Area")
- Widget click only opens app — should deep-link to relevant screen
- Geocoding called every update even if coordinates unchanged
